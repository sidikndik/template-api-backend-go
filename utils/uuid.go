...
// ConvertSliceUUIDToStrFormat func
// format: example -> '%s'
func ConvertSliceUUIDToStrFormat(listUuid []guuid.UUID, separator string, format string) (result string) {
	if len(listUuid) < 1 {
		return ""
	}
...
func RemoveDuplicatedUUID(listUUID []guuid.UUID) (result []guuid.UUID) {
	if len(listUUID) == 0 {
		return
	}
...
func FindSliceUUID(slice []guuid.UUID, val guuid.UUID) (index int, isFound bool) {
